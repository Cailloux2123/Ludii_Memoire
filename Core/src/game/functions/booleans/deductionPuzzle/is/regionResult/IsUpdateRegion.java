 ...
 */
@Hide
public class IsUpdateRegion extends BaseBooleanFunction
{
	...
	public boolean eval(final Context context)
	{
		...
		Regions[] regions = context.equipment().regions();
		for (Regions region : regions) {
			int[] sites2 = region.sites();
			for (int site : sites2) {
				if (!cs.isResolved(site, realType)) {
					// ici je dois remove la region
				}
			}
		}
		...
		if (toKeep.size() == result) {
			//Je dois ici créer une nvl région mais du coup je la stocke dans quoi ?
		}
