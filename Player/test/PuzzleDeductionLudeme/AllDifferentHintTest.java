...
import game.Game;
import game.functions.booleans.deductionPuzzle.all.AllDifferentHint;
...
/**
 * For Puzzle: A Unit Test to test the all HintDifferent ludeme.
 * 
 * @author Pierre.Accou
 */
@SuppressWarnings("static-method")
public class AllDifferentHintTest
{
	/**
	 * The test to run
	 * 
	...
	{
		System.out.println(
				"\n=========================================\nAll Hint Different Ludeme Deduction Puzzle Test\n=========================================\n");

		final long startAt = System.nanoTime();
		final File startFolder = new File("../Common/res/lud/puzzle/deduction/Test/AllHintDifferent");
		...
		for (final File fileEntry : entries)
		{
			if (fileEntry.getName().contains(".lud"))
			{
				...
				final File trialsDir = new File("../Player/res/random_trials/puzzle/deduction/Test/AllHintDifferent");
				...
				for (final File trialFile : trialFiles)
				{
					...
					AllDifferentHint allDifferentHint = new AllDifferentHint(null, null, null, null);
					
					allDifferentHint.eval(context);
					
					if (trialFile.getName().contains("True")) {
						assert(allDifferentHint.eval(context) == true) : "The ludeme AtMost will return true for this case but return false";
					}
					else {
						assert(allDifferentHint.eval(context) != true) : "The ludeme AtMost will return false for this case but return true";;
					}
