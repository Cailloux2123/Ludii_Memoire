...
/**
 * For Puzzle: A Unit Test to test the is Sum ludeme.
 * 
 * @author Pierre.Accou
 */
...
public class IsSumTest
{
	...
	{
		System.out.println(
				"\n=========================================\nIs Sum Ludeme Deduction Puzzle Test\n=========================================\n");

		final long startAt = System.nanoTime();
		final File startFolder = new File("../Common/res/lud/puzzle/deduction/Test/IsSum");
		...
		for (final File fileEntry : entries)
		{
			if (fileEntry.getName().contains(".lud"))
			{
				...
				final File trialsDir = new File("../Player/res/random_trials/puzzle/deduction/Test/IsSum");
				...
				for (final File trialFile : trialFiles)
				{
					...
					IsSum isSum = new IsSum(null, null, null, intconstante);
					
					if (trialFile.getName().contains("True")) {
						assert(isSum.eval(context) == true) : "The ludeme IsSum will return true for this case but return false";
					}
					else {
						assert(isSum.eval(context) != true) : "The ludeme IsSum will return false for this case but return true";
					}
